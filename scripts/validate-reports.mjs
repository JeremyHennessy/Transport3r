...
try{
 for(const scenario of ['rich','large','small','inactive','no-inspections','partial','multiple-vins','history'])for(const format of ['brief','detailed']){
  ...
  assert.ok(dom.includes('data-testid="carrier-report"'),`${name}: report did not render`);assert.ok(!dom.includes('data-report-error='),`${name}: runtime error`);assert.ok(!dom.includes('could not render this view'),name);assert.ok(dom.includes(`REPORT FIXTURE ${scenario}`),name);
