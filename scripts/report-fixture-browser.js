(()=>{
 ...
 window.fetch=async(input,options)=>{
  const text=String(input);if(text.includes('data.transportation.gov/api/views/'))return Response.json({id:text.split('/').at(-1).replace('.json',''),rowsUpdatedAt:1,viewLastModified:1,tableId:1,columns:[]});
  ...
  const offset=Number(url.searchParams.get('$offset')??0),limit=Number(url.searchParams.get('$limit')??5000);return Response.json(rows.slice(offset,offset+limit).map((row,index)=>({...row,transport_row_id:`${id}:${offset+index}`})));
 };
})();
