...
const carriers=await Promise.all(['80806','54283','264184','265752'].map(async dot=>{
  try {
    const [census,inspections]=await Promise.all([
      app.queryByDot(registry,'az4n-8mr2',dot,{limit:2}),
      app.queryByDot(registry,'fx4q-ay7w',dot,{limit:500,includeTotal:true,orderAliases:['INSP_DATE']}),
    ]);
