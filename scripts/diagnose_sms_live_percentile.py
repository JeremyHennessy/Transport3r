...
pages do publish the official percentile. This diagnostic therefore:

1. Builds the full passenger comparison population for each BASIC from current bulk
   output.
2. Samples carriers that should have enough events to receive a percentile.
3. Fetches the corresponding live FMCSA BASIC page and parses official measure,
   percentile and snapshot ending date.
4. Tests multiple tie-ranking and rounding transforms against the published result.

This is diagnostic-only. It fails unless enough official percentiles are retrieved, but
it does not automatically bless a percentile formula. A follow-up strict validator
must promote a single supported transform before Transport3r displays calculated
property-carrier percentiles.
"""
...
def fetch_bytes(url: str, timeout: int = 35, attempts: int = 3) -> bytes:
    ...
    for attempt in range(1, attempts + 1):
        ...
        except Exception as exc:  # noqa: BLE001 - diagnostic logs need exact upstream failure
            last_error = exc
            if attempt < attempts:
...
def scaled_percentiles(values: list[float], method: str) -> list[float]:
    ...
    if method == "dense":
        ...
        maximum = max(ranks)
        return [0.0 if maximum == 0 else 100.0 * rank / maximum for rank in ranks]
    ranks = tie_ranks(values, method)
    denominator = max(1, len(values) - 1)
    return [100.0 * rank / denominator for rank in ranks]
...
def quantile_sample(rows: list[dict[str, Any]], count: int) -> list[dict[str, Any]]:
    ...
    dedup: dict[str, dict[str, Any]] = {str(row["dot_number"]): row for row in selected}
    ...
    for basic_key, rule in BASICS.items():
        ...
        for group, population in populations.items():
            ...
            for method in ("min", "max", "average", "dense"):
                ...
                rank_maps[(group, method)] = {
                    str(row["dot_number"]): pct for row, pct in zip(population, pct_values, strict=True)
                }

        candidates.sort(key=lambda row: (row["_group"], row["_measure"], str(row["dot_number"])))
        sample = quantile_sample(candidates, 28)
        ...
        for observation in observations:
            official = observation["official_percentile"]
            for name, predicted in observation["predictions"].items():
                method_errors[name].append(abs(predicted - official))
