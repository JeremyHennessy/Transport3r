...
The large-carrier live gate retains up to three acquisition attempts for transport failures, HTTP 429/5xx and unavailable totals. Each retry is a fresh observation, with attempt outcomes retained in the artifact. Invalid payloads and identity/count validation failures are not retried; exhausted acquisition still fails the gate. Application request behavior is unchanged.

SMS source-state acquisition applies the same three-attempt bound to connection/time-out errors and HTTP 429/5xx, preserving attempt timestamps and errors. Invalid metadata is not retried. Tests retain rejection of unavailable sources, malformed metadata, source changes and observations outside the query window; the source-cut requirements are unchanged.
