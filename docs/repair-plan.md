...
The [Evidence status audit](evidence-status-audit.md) now flags missing/partial Evidence sweeps, distinguishes authority source failures from empty history, and explains rejected SMS inputs or conflicting official outputs in the existing warning presentation. This closes those identified renderer gaps; it does not certify every source or scoring release gate.

The [carrier table filters](carrier-table-filters.md) add numeric reported-driver screening and risk-score availability without releasing a score. Follow-up [request recovery](carrier-query-recovery.md) makes Retry and repeated filter actions refetch, cancels obsolete requests, bounds body loading and rejects stale responses. A production-bundle browser gate retains these recovery checks.
