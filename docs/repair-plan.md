...
SMS runtime input validation now rejects invalid weights, duplicated parents, orphan violations and carrier contamination. CI compares the actual application TypeScript to retained official records. [Population selection](sms-population-selection.md) now distinguishes general and passenger-specific AB/C outputs, rejects unresolved population conflicts and keeps comparisons without a denominator unavailable. See [SMS replay integrity](sms-replay-integrity.md). Full-population and temporal alignment validation remain separate gates.
