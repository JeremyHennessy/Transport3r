...
SMS runtime input validation now rejects invalid weights, duplicated parents, orphan violations and carrier contamination. CI compares the actual application TypeScript to the same official records used by the Python regression. See [SMS replay integrity](sms-replay-integrity.md); official population selection and temporal alignment remain separate gates.
