...
function SourcesPage({ health, schema }: { health: SourceHealthPayload | null; schema: SchemaRegistry | null }) {
  ...
  return <main className="t3-main"><PageHeading eyebrow="Data lineage & operating contract" title="Data Sources" copy="All 27 configured FMCSA/DOT datasets, why they exist in Transport3r, their update cadence, and the job each performs in underwriting or validation." action={<div className="t3-asof"><span>Health snapshot</span><strong>{formatDateTime(health?.generated_at)}</strong></div>} />
